...
; Ask for a selection
    ...
    if (plotlog) then begin 
            ...
            plotvalue = 1.00*value[nGcs - 1:nAlts-nGcs - 1]
       ...
       plot, plotvalue, plotalts, $
             ...
             yrange = [505.0, max(plotAlts)], ystyle = 1, /XLOG
       ...
       plot, plotvalue, plotalts, $
             background = 7, color = 0, thick = 2.0, $
             xrange = [min(plotvalue), max(plotvalue)], xstyle = 1, $
             yrange = [505.0, max(plotalts)], ystyle = 1
...
;==============Filled Circles
        ...
        if (iData eq 1) then begin
           ...
           restore, filename = 'ColdBinned_INMS_Data.save'
