...
;;;; WARNING LIMITS ARE HARD CODED FOR ANNA'S STUFF
;;;; JMB 12-14-2014

pro derpy_gui_event,event
...
;            print,'nfiles:',nfiles
            if filelist[0] eq '' or nfiles eq 0 then begin
                ...
                ;print, filelist
                read_thermosphere_file, filelist, nvars, nalts, nlats, $
                                  nlons,vars,data,rb,cb,bl_cnt, iTime

                Sigma_Hall     = fltarr(nLons, nLats, nAlts)
                Sigma_Pederson = fltarr(nLons, nLats, nAlts)
                Sigma_Hall = 0.0
                Sigma_Pderson = 0.0
                calc_derpy_conductance, filelist, Sigma_Hall, Sigma_Pederson 

                jmbdata = fltarr(nvars+1,nlons,nlats,nalts)
                jmbvars = strarr(nvars+1)

                jmbdata[0:nvars-1,0:nlons-1,0:nlats-1,0:nalts-1]=$
                   data[0:nvars-1,0:nlons-1,0:nlats-1,0:nalts-1] 
                jmbvars[0:nvars-1] = vars[0:nvars-1]

                jmbdata[nvars,0:nlons-1,0:nlats-1,0:nalts-1] = Sigma_Pederson

                ;jmbvars[nvars] = 'Skaderp'
                ;jmbvars[nvars] = textoidl('\Sigma_{P} (Mho)')
                jmbvars[nvars] = '(Mho)'

                data = fltarr(nvars+1,nlons,nlats,nalts)
                vars = strarr(nvars+1)
...
;                    print,'n is:',n
                    ...
                    Sigma_Hall = 0.0
                    Sigma_Pderson = 0.0
                    calc_derpy_conductance, mulfiles(0), Sigma_Hall, Sigma_Pederson 
...
;                        print,nvars,nalts,nlats,nlons
                        ...
                        if n gt 1 then begin
                            for i = 0,n-1 do begin
                                ...
                                Sigma_Hall = 0.0
                                Sigma_Pderson = 0.0
                                calc_derpy_conductance, mulfiles(i), Sigma_Hall, Sigma_Pederson 
