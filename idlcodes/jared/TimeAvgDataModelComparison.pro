...
; Ask for a selection
    ...
    if (plotlog) then begin 
       ...
       plot, plotvalue, plotalts, $
             background = 7, color = 0, thick = 2.0, $
             xrange = [min(plotvalue), max(plotvalue)], xstyle = 1, $
             yrange = [485.0, max(plotalts)], ystyle = 1
...
;==============Filled Circles
        ...
        if (iData eq 1) then begin
           ...
           restore, filename = '3DBinned_INMS_Data.save'

            AltMask = where(ModelBinCenters ge 1050.0)
   ...
   ;; Reduce the Data to the values above 1100 km
        ...
        if (iData eq 6) then begin
            ...
            NewDataCH4iso = 1.07*Bin_CH4isoRatio[NewIndex]

            NewDataArray = 1.00*NewDataCH4iso
            NewDataAlts = Bin_altitude[NewIndex]
...
;; ================ H2 FlUXES
     ...
     print, 'H2 Upper Boundary Fluxes:', $
                 0.5*( TimeAveData[iH2_,FluxIndexUpper1]*TimeAveData[iH2Vel_,FluxIndexUpper1]*((TimeAveData[2,FluxIndexUpper1] + 2575.0*1000.0 )/(2575.0*1000.0) )^2.0 + $
                       TimeAveData[iH2_,FluxIndexUpper2]*TimeAveData[iH2Vel_,FluxIndexUpper2]*((TimeAveData[2,FluxIndexUpper2] + 2575.0*1000.0 )/(2575.0*1000.0) )^2.0 ), ' (1/m^2/s) ', $
                      0.5*( TimeAveData[iH2_,FluxIndexUpper1]*TimeAveData[iH2Vel_,FluxIndexUpper1]*4.0*!pi*(TimeAveData[2,FluxIndexUpper1] + 2575.0*1000.0)^2.0 + $
                            TimeAveData[iH2_,FluxIndexUpper2]*TimeAveData[iH2Vel_,FluxIndexUpper2]*4.0*!pi*(TimeAveData[2,FluxIndexUpper2] + 2575.0*1000.0)^2.0 ),  ' (molecules/s) ', $
                       0.5*(TimeAveData[iH2_,FluxIndexUpper1]*TimeAveData[iH2Vel_,FluxIndexUpper1]*2.0*4.0*!pi*(TimeAveData[2,FluxIndexUpper1] + 2575.0*1000.0)^2.0 + $
                            TimeAveData[iH2_,FluxIndexUpper1]*TimeAveData[iH2Vel_,FluxIndexUpper1]*2.0*4.0*!pi*(TimeAveData[2,FluxIndexUpper1] + 2575.0*1000.0)^2.0 ), ' (amu/s) '

;; ============= END H2 FLUXES

;; ================ CH4 FlUXES
     print, 'CH4 Upper Boundary Fluxes:', $
                 0.5*( TimeAveData[iCH4_,FluxIndexUpper1]*TimeAveData[iCH4Vel_,FluxIndexUpper1]*((TimeAveData[2,FluxIndexUpper1] + 2575.0*1000.0 )/(2575.0*1000.0) )^2.0 + $
                       TimeAveData[iCH4_,FluxIndexUpper2]*TimeAveData[iCH4Vel_,FluxIndexUpper2]*((TimeAveData[2,FluxIndexUpper2] + 2575.0*1000.0 )/(2575.0*1000.0) )^2.0 ), ' (1/m^2/s) ', $
                      0.5*( TimeAveData[iCH4_,FluxIndexUpper1]*TimeAveData[iCH4Vel_,FluxIndexUpper1]*4.0*!pi*(TimeAveData[2,FluxIndexUpper1] + 2575.0*1000.0)^2.0 + $
                            TimeAveData[iCH4_,FluxIndexUpper2]*TimeAveData[iCH4Vel_,FluxIndexUpper2]*4.0*!pi*(TimeAveData[2,FluxIndexUpper2] + 2575.0*1000.0)^2.0 ),  ' (molecules/s) ', $
                       0.5*(TimeAveData[iCH4_,FluxIndexUpper1]*TimeAveData[iCH4Vel_,FluxIndexUpper1]*16.0*4.0*!pi*(TimeAveData[2,FluxIndexUpper1] + 2575.0*1000.0)^2.0 + $
                            TimeAveData[iCH4_,FluxIndexUpper1]*TimeAveData[iCH4Vel_,FluxIndexUpper1]*16.0*4.0*!pi*(TimeAveData[2,FluxIndexUpper1] + 2575.0*1000.0)^2.0 ), ' (amu/s) '

;; ============= END CH4 FLUXES

;; ================ N2 FlUXES
     print, 'N2 Upper Boundary Fluxes:', $
                 0.5*( TimeAveData[iN2_,FluxIndexUpper1]*TimeAveData[iN2Vel_,FluxIndexUpper1]*((TimeAveData[2,FluxIndexUpper1] + 2575.0*1000.0 )/(2575.0*1000.0) )^2.0 + $
                       TimeAveData[iN2_,FluxIndexUpper2]*TimeAveData[iN2Vel_,FluxIndexUpper2]*((TimeAveData[2,FluxIndexUpper2] + 2575.0*1000.0 )/(2575.0*1000.0) )^2.0 ), ' (1/m^2/s) ', $
                      0.5*( TimeAveData[iN2_,FluxIndexUpper1]*TimeAveData[iN2Vel_,FluxIndexUpper1]*4.0*!pi*(TimeAveData[2,FluxIndexUpper1] + 2575.0*1000.0)^2.0 + $
                            TimeAveData[iN2_,FluxIndexUpper2]*TimeAveData[iN2Vel_,FluxIndexUpper2]*4.0*!pi*(TimeAveData[2,FluxIndexUpper2] + 2575.0*1000.0)^2.0 ),  ' (molecules/s) ', $
                       0.5*(TimeAveData[iN2_,FluxIndexUpper1]*TimeAveData[iN2Vel_,FluxIndexUpper1]*28.0*4.0*!pi*(TimeAveData[2,FluxIndexUpper1] + 2575.0*1000.0)^2.0 + $
                            TimeAveData[iN2_,FluxIndexUpper1]*TimeAveData[iN2Vel_,FluxIndexUpper1]*28.0*4.0*!pi*(TimeAveData[2,FluxIndexUpper1] + 2575.0*1000.0)^2.0 ), ' (amu/s) '
