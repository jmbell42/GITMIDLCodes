...
type_2 = fix(ask('second field to plot (under first, 0 for none)','0'))-1
...
if (type_2 ge 0) then begin
  ...
  if (strpos(mklower(fields(type_2)),'electric field') gt -1) then 	$
    units = '(mV/m)' 
  if (strpos(mklower(fields(type_2)),'current') gt -1) then 		$
    units = '(A/m!E2!N)' 
...
for i = n_start, n_end do begin
  mini_array(i-n_start) = min(data_1(i,*,*)-back_1)
  maxi_array(i-n_start) = max(data_1(i,*,*)-back_1)
endfor
...
factor  = 1.0
while (range_1 gt dc*20.0*factor) do factor=factor+0.5
dc = factor*dc
...
for i = n_start, n_end do begin
  mini_array(i-n_start) = min(data_2(i,*,*)-back_2)
  maxi_array(i-n_start) = max(data_2(i,*,*)-back_2)
endfor
...
if (mini_2 ge 0.0) then begin
  mini_2 = 0.0
  range_2 = maxi_2
endif else range_2 = max([abs(mini_2),maxi_2])*2.0
if (range_2 gt 0.0) then begin
  dc      = 10.0^fix(alog10(range_2/20.0))
  factor  = 1.0
  while (range_2 gt dc*20.0*factor) do factor=factor+0.5
  dc = factor*dc
  dc = float(ask('contour level for '+strcompress(fields(type_2)),	$
                 strcompress(string(dc))))
  if (mini_2) eq 0.0 then 						$
    levels_2 = findgen(21)*dc						$
  else levels_2 = (findgen(21) - 10.0)*dc
endif else levels_2 = findgen(21)/20.0-1.1
...
;--------------------------------------------------------------
  ...
  if (not vect) then begin
    ...
    length = (levels_1(1) - levels_1(0))*10.0
    title = string(length, format='(f5.2)')
    normal = 1.0/(levels_1(1) - levels_1(0))
