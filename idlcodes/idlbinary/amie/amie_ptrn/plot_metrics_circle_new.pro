...
IntFluxBinned  = fltarr(nMLTs, nAE)

FluxBinned_mag     = fltarr(nMAGs, nAE)
...
IntFluxBinned_mag  = fltarr(nMAGs, nAE)

psfile = ask('ps file','metrics.ps')
...
;      if (metrics(7,j,i) gt 18.5 or metrics(7,j,i) lt 2.5) then begin
      if (mlt eq 0 or mlt ge nMLTs-2) then begin
        ...
        FluxBinned_mag(mag,ae)  = FluxBinned_mag(mag,ae)  + $
	  metrics(0,j,i)*metrics(3,j,i)
        ccBinned_amie_mag(mag,ae)  = ccBinned_amie_mag(mag,ae)  + $
	  metrics(2,j,i)*metrics(3,j,i)
        CCBinned_mag(mag,ae)    = CCBinned_mag(mag,ae)    + metrics(3,j,i)
	  ...
	  metrics(5,j,i)*metrics(3,j,i)
        IntFluxBinned_mag(mag,ae)  = IntFluxBinned_mag(mag,ae)  + $
	  metrics(12,j,i)*metrics(3,j,i)
...
FluxBinned(loc)  = FluxBinned(loc) / CCBinned(loc)
...
IntFluxBinned(loc)  = IntFluxBinned(loc) / CCBinned(loc)

CCBinned(loc)    = CCBinned(loc) / nPts(loc)

loc = where(CCBinned_mag gt 0)

FluxBinned_mag(loc)  = FluxBinned_mag(loc) / CCBinned_mag(loc)
...
IntFluxBinned_mag(loc)  = IntFluxBinned_mag(loc) / CCBinned_mag(loc)
...
levels = findgen(26)*200.0/25.0 - 100.0
clevels = findgen(26)*ncolors/25.0
contour, FluxBinned(locnpts), mlt(locnpts), ae(locnpts), /follow, $
	levels = levels, c_color = clevels, xtickname=strarr(10)+' ', $
	pos = pos, /noerase, ystyle = 1, xstyle = 1, /fill, $
	ytitle = 'AE (nT)',/irr, xrange=[0,24], yrange = [0,max(ae)]

contour, FluxBinned(locnpts), mlt(locnpts), ae(locnpts), /follow, levels = levels, $
	pos = pos, /noerase, ystyle = 5, xstyle = 5,/irr, xrange=[0,24], yrange = [0,max(ae)]
...
ctpos = [pos(2)+0.01,pos(1),pos(2)+0.05,pos(3)]
plotct, ncolors, ctpos, mm(levels), '% Diff Flux', /right
...
levels = findgen(26)*200.0/25.0 - 100.0
clevels = findgen(26)*ncolors/25.0
contour, IntFluxBinned(locnpts), mlt(locnpts), ae(locnpts), /follow, levels = levels, c_color = clevels, $
	pos = pos, /noerase, ystyle = 1, xstyle = 1, /fill, $
	ytitle = 'AE (nT)', xtitle = 'MLT',/irr, xrange=[0,24], yrange = [0,max(ae)]

contour, IntFluxBinned(locnpts), mlt(locnpts), ae(locnpts), /follow, levels = levels, $
	pos = pos, /noerase, ystyle = 5, xstyle = 5,/irr, xrange=[0,24], yrange = [0,max(ae)]
...
ctpos = [pos(2)+0.01,pos(1),pos(2)+0.05,pos(3)]
plotct, ncolors, ctpos, mm(levels), '% Diff Int Flux', /right
