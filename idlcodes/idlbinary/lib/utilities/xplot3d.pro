; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/utilities/xplot3d.pro#1 $
;
...
if (keyword_set(overplot)) then begin
    ...
    if (wid ne 0l) then begin
        ...
        (*pState).oXaxis->GetProperty, XCOORD_CONV=xs, YCOORD_CONV=ys, $
            ZCOORD_CONV=zs, RANGE=xaxrange
        (*pState).oYaxis->GetProperty, RANGE=yaxrange
        (*pState).oZaxis->GetProperty, RANGE=zaxrange
...
;       feature. This feature is private to RSI, and is not guaranteed to
;       remain in IDL or to remain unchanged. This features is undocumented
;       because is not considered permanent. Research Systems reserves the
