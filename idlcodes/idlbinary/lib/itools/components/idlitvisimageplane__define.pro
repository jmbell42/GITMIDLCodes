; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/itools/components/idlitvisimageplane__define.pro#1 $
;
...
;   or a 0 otherwise.
;-
function IDLitVisImagePlane::Init, $
    ...
    if (not KEYWORD_SET(name)) then name ="ImagePlane"
    if (not KEYWORD_SET(description)) then description ="An Image Plane"

    ; Initialize superclasses.
    if (self->IDLitVisualization::Init(NAME=name, $
        TYPE='IDLIMAGE PLANE', $
        ICON='image', $
        DESCRIPTION=description, $
        /MANIPULATOR_TARGET, $
    ...
    ;; Make this one insensitive so that it does not show up in the
    ;; style sheet.  We don't want this changed when a style is applied.
    ...
    self._oPolygon->SetPropertyAttribute, ['AMBIENT', $
                                           'DIFFUSE', $
                                           'SPECULAR', $
                                           'EMISSION', $
                                           'SHININESS', $
...
; INPUTS:
;-
pro IDLitVisImagePlane::_CreateGraphics
...
; INPUTS:
;-
pro IDLitVisImagePlane::_InitImagePlane
    ...
    if OBJ_VALID(self._oVolume) then begin
        ...
        ;; Get and save dims for easy access later on
        ...
        self->IDLgrModel::Scale, displayDims[0], displayDims[1], displayDims[2]
        t = displayLoc + displayDims/2.0
...
pro IDLitVisImagePlane::_UpdateImagePlane
    ...
    ;; Defer doing all this work until all parm set parms are processed
    if self._parmSetChange ne 0 then return
    ...
    if N_ELEMENTS(data0) gt 0 then begin
        ...
        center = (self._center > 0) < self._volDims
        ...
        2: begin
            if ARRAY_EQUAL(self._center, center) then begin
        ...
        img[*, 0:self._xSize-1, 0:self._ySize-1] = REFORM(im, 4, self._xSize, self._ySize)
        ...
        if OBJ_VALID(self._oImageParm) then $
            success = self._oImageParm->SetData(REFORM(im, 4, self._xSize, self._ySize), 'IMAGEPIXELS')
        self._parmUpdate = 0
    endif
end
...
pro IDLitVisImagePlane::OnDataChangeUpdate, oSubject, parmName
    ...
    if self._parmUpdate then return
    ...
    case STRUPCASE(parmName) of
        '<PARAMETER SET>': begin
            ...
            ;; Postpone updating the image plane until parmset is done
            self._parmSetChange = 1
            ...
            self._parmSetChange = 0
            ;; Now update the image plane just once
            self->_UpdateImagePlane
            end
        ...
        'IMAGEPIXELS': begin
         ...
         'VOLUME0':self->_InitImagePlane
         'VOLUME1':self->_InitImagePlane
         'VOLUME2':self->_InitImagePlane
         'VOLUME3':self->_InitImagePlane
         'RGB_TABLE0':self->_UpdateImagePlane
         'RGB_TABLE1':self->_UpdateImagePlane
         'OPACITY_TABLE0':self->_UpdateImagePlane
         'OPACITY_TABLE1':self->_UpdateImagePlane
...
pro IDLitVisImagePlane__Define
    ...
    struct = { IDLitVisImagePlane,          $
        ...
        _parmUpdate: 0b,                $ ; In progress
        _parmSetChange: 0b,             $ ; True if OnDataChangeUpdate is
                                        $ ; processing a parmset
        _oImage: OBJ_NEW(),             $ ; IDLgrImage object for texture map
        _oImageParm: OBJ_NEW(),         $ ; IDLitDataImage object (image parm)
        _oPolygon: OBJ_NEW(),           $ ; IDLgrPolygon object
        _oVolume: OBJ_NEW()             $ ; IDLgrVolume object
    }
end
