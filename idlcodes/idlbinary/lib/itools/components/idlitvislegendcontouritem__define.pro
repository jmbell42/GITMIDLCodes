; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/itools/components/idlitvislegendcontouritem__define.pro#1 $
;
...
PRO IDLitVisLegendContourItem::UpdateLevels
    ...
    for i=0, nLevels-1 do begin
        ...
        oLevelItem = OBJ_NEW('IDLitVisLegendContourLevelItem', $
            HIDE=~self._showLevels)
        oLevelItem->_SetTool, self->GetTool()
        self->Add, oLevelItem
        oLevelItem->BuildItem, self._oVisTarget, i
    endfor
...
;----------------------------------------------------------------------------
PRO IDLitVisLegendContourItem::BuildItem, oTarget
...
pro IDLitVisLegendContourItem::SetProperty,  $
    ...
    IF n_elements(levelLabels) NE 0 THEN BEGIN
      ...
      IF levelLabels GT 0 THEN BEGIN
        oItems = self->Get(/ALL,ISA='IDLITVISLEGENDCONTOURLEVELITEM')
        FOR i=0,n_elements(oItems)-1 DO oItems[i]->SetProperty, $
          MY_LABEL_VALUE=levelLabels-1
      ENDIF
    ENDIF
