; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/itools/components/idlitvisplot__define.pro#1 $
;
...
pro IDLitVisPlot::_RegisterProperties, $
    ...
    if (registerAll) then begin
        ...
        self._oPlot->RegisterProperty, 'TRANSPARENCY', /INTEGER, $
    ...
    if (registerAll || (updateFromVersion lt 610)) then begin
        ...
        ; Use TRANSPARENCY and PLOT_TRANSPARENCY properties instead.
...
function IDLitVisPlot::Init, $
    ...
    if(self->IDLitVisualization::Init(/REGISTER_PROPERTIES, $
        DEPTH_TEST_FUNCTION=4, $
        HELP=help, $
        TOOL=tool, $
        NAME=KEYWORD_SET(name) ? name : 'Plot', $
        ICON=KEYWORD_SET(icon) ? icon : 'plot', $
        TYPE=KEYWORD_SET(type) ? type : 'IDLPLOT',$
        DESCRIPTION=KEYWORD_SET(description) ? description : "A Plot Visualization", $
...
pro IDLitVisPlot::GetProperty, $
    ...
    PLOT_TRANSPARENCY=plotTransparency, $
...
pro IDLitVisPlot::SetProperty, $
    ...
    if (N_ELEMENTS(fillBackground)) then begin
        ...
        self->SetPropertyAttribute, $
            ['FILL_COLOR', 'FILL_LEVEL', 'TRANSPARENCY'], $
