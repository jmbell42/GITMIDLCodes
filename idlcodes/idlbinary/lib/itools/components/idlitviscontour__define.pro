; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/itools/components/idlitviscontour__define.pro#1 $
;
...
pro IDLitVisContour::SetProperty, $
    ...
    if (N_ELEMENTS(minValue) || N_ELEMENTS(maxValue)) then begin
        self._oContour->SetProperty, MIN_VALUE=minValue, $
            MAX_VALUE=maxValue
        self->_UpdateDefaultIndices
        self->OnDataChange, self
        self->OnDataComplete, self
    ...
    if (N_ELEMENTS(zvalue) gt 0) then begin
        self._oContour->GetProperty, DATA_VALUES=dataValues
        self._zvalue = zvalue
        IF (N_ELEMENTS(dataValues) GT 1) THEN BEGIN
            self._oContour->SetProperty, GEOMZ=zvalue
    ...
    if (N_ELEMENTS(planar) gt 0) then begin
        ...
        self._oContour->GetProperty, DATA_VALUES=dataValues
        ; If we change PLANAR, we may need to change what is stored within
        ; GEOMZ.
        if (N_ELEMENTS(dataValues) gt 1) then begin
            self._oContour->SetProperty, $
                GEOMZ=KEYWORD_SET(planar) ? self._zvalue : dataValues
            ; put the visualization into 3D mode if necessary
            self->Set3D, ~planar || (self._zvalue ne 0), /ALWAYS
            self->OnDataChange, self
            self->OnDataComplete, self
...
;            endif
        ...
        self->SetPropertyAttribute, 'ZVALUE', $
            SENSITIVE=KEYWORD_SET(planar)
    endif
    ...
    case STRUPCASE(parmName) of
        ...
        'Z': BEGIN
          ...
          self._oContour->SetProperty, DATA_VALUES=zData, $
              ...
              GEOMX=xData, $
              GEOMY=yData, $
              GEOMZ=KEYWORD_SET(planar) ? self._zvalue : zData
        ...
        'X': BEGIN
            oDataSpace = self->GetDataSpace(/UNNORMALIZED)
            if (OBJ_VALID(oDataSpace)) then begin
                oDataSpace->GetProperty, X_LOG=xLog
                self._xVisLog = xLog
            endif
            xData = self->_GetXYdata('X')
            if (N_ELEMENTS(xData) gt 1) then begin
        ...
        'Y': BEGIN
            oDataSpace = self->GetDataSpace(/UNNORMALIZED)
            if (OBJ_VALID(oDataSpace)) then begin
                oDataSpace->GetProperty, Y_LOG=yLog
                self._yVisLog = yLog
            endif
            yData = self->_GetXYdata('Y')
            if (N_ELEMENTS(yData) gt 1) then begin
