; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/itools/framework/_idlitmanipulator__define.pro#1 $
;
...
function _IDLitManipulator::Init, $
   ...
   self._defaultCursor = 'ARROW'
    ...
    if(n_elements(BUTTON_EVENTS) eq 0)then $
      BUTTON_EVENTS=1
    if(n_elements(MOTION_EVENTS) eq 0)then $
      MOTION_EVENTS=1
    if(n_elements(KEYBOARD_EVENTS) eq 0)then $
      KEYBOARD_EVENTS=1
    ...
    self->_IDLitManipulator::SetProperty, $
                          BUTTON_EVENTS=BUTTON_EVENTS, $
                          MOTION_EVENTS=MOTION_EVENTS, $
                          KEYBOARD_EVENTS=KEYBOARD_EVENTS, $
                          _extra=_extra
...
;   this routine will manage setting the cursor on the window. This
;
; Parameters
...
;
; Purpose:
;   Gets the current event mask for the manipulator.
;
function _IDLitManipulator::_GetEventMask, _REF_EXTRA=_extra

   compile_opt idl2, hidden

   if(n_elements(_extra) gt 0)then $
      QUERY_EVENT_MASK, self._uiEventMask, _EXTRA=_extra
...
;
; Purpose:
;   Used to set the event mask for the manipulator.
;
; Keywords:
;   - Anything accepted by MAKE_EVENT_MASK
;
;   - TRANSIENT_MOTION - If set, will cause motion events to be
;                        enabled on a mouse down, and disabled
;                        on a mouse up (if they were not originally
;                        enabled.
;
pro _IDLitManipulator::_SetEventMask, iuMask, _EXTRA=extra, $
                     TRANSIENT_MOTION=TransMotion

   compile_opt idl2, hidden

   ; check transient motion
   if(n_elements(TransMotion) gt 0)then $
       self->SetProperty, transient_motion=transient_motion

   ;Combine the bits from the provided mask, if any, and those indicated
   ; by the keywords.
   self._uiEventMask = ((N_ELEMENTS(iuMask) ne 0) ? iuMask : 0UL) or $
                          MAKE_EVENT_MASK(_EXTRA=extra)
end
...
pro _IDLitManipulator::GetProperty, $
   ...
   if(arg_present(BUTTON_EVENTS) or $
      arg_present(MOTION_EVENTS) or $
      arg_present(KEYBOARD_EVENTS))then $
...
pro _IDLitManipulator::SetProperty, $
   ...
   if(n_elements(BUTTON_EVENTS))then begin
       if(keyword_set(BUTTON_EVENTS))then $ ; add in bits
         self._uiEventMask = self._uiEventMask or $
                make_event_mask(/button_events) $
       else $                   ;mask out
         self._uiEventMask = self._uiEventMask and $
              (make_event_mask(/button_events) xor 'ffff'xu)
   endif

   if(n_elements(MOTION_EVENTS))then begin
       if(keyword_set(MOTION_EVENTS))then $ ; add in bits
         self._uiEventMask = self._uiEventMask or $
                make_event_mask(/motion_events) $
       else $                   ;mask out
         self._uiEventMask = self._uiEventMask and $
              (make_event_mask(/motion_events) xor 'ffff'xu)
   endif

   if(n_elements(KEYBOARD_EVENTS))then begin
       if(keyword_set(KEYBOARD_EVENTS))then $ ; add in bits
         self._uiEventMask = self._uiEventMask or $
                make_event_mask(/keyboard_events) $
       else $                   ;mask out
         self._uiEventMask = self._uiEventMask and $
              (make_event_mask(/keyboard_events) xor 'ffff'xu)
   endif
