; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/itools/framework/idlitvisnormalizer__define.pro#1 $
;
...
pro IDLitVisNormalizer::_RegisterProperties, $
    ...
    if (registerAll || (updateFromVersion lt 610)) then begin
        self->RegisterProperty, 'ANISOTROPIC_SCALE_2D', $
            NAME='Anisotropic 2D scale', $
            DESCRIPTION='Presentation scale factor applied along Y axis', $
            /FLOAT, $
            VALID_RANGE=[0.1d,1.0d,0.1d]

        self->RegisterProperty, 'ANISOTROPIC_SCALE_3D', $
...
pro IDLitVisNormalizer::SetProperty, $
    ...
    if (N_ELEMENTS(anisotropicScale2D) gt 0) then begin
        newScale = anisotropicScale2D[0] > 0.001
        if (self._anisotropicScale2D ne newScale) then begin
    ...
    if (N_ELEMENTS(anisotropicScale3D) gt 0) then begin
        newScale = anisotropicScale3D[0] > 0.001
        if (self._anisotropicScale3D ne newScale) then begin
