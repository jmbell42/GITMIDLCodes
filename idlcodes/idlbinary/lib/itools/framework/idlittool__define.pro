; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/itools/framework/idlittool__define.pro#1 $
;
...
pro IDLitTool::OnSelectionChange, oWin
    ...
    ; See if GROUP will work.
    nSel = N_ELEMENTS(oSel)
    if (nSel ge 2) then begin
        ; Get the first parent.
        if (OBJ_VALID(oSel[0])) then $
            oSel[0]->GetProperty, _PARENT=oParent
        if (OBJ_VALID(oParent)) then begin
            ; If all the other parents match then
            ; grouping is allowed.
            for i=1,nSel-1 do begin
                if (~OBJ_VALID(oSel[i])) then begin
                    oParent = OBJ_NEW()  ; dead obj
                    break
                endif
                oSel[i]->GetProperty, _PARENT=oOtherParent
                if (oOtherParent ne oParent) then begin
                    oParent = OBJ_NEW()  ; mismatched parents
                    break
                endif
            endfor
        endif
    ...
    id = self->GetFullIdentifier()+"/OPERATIONS/EDIT/GROUPING/GROUP"
    self->DoOnNotify, id, 'SENSITIVE', OBJ_VALID(oParent)

    ; Now check for UNGROUP.
    allowUngroup = 0
    for i=0,nSel-1 do begin
        ; See if any of my selected items is a viz group.
        if (OBJ_ISA(oSel[i], 'IDLitVisGroup')) then begin
            allowUngroup = 1  ; I'm in business
            break
        endif
    endfor
    id = self->GetFullIdentifier()+"/OPERATIONS/EDIT/GROUPING/UNGROUP"
    self->DoOnNotify, id, 'SENSITIVE', allowUngroup

    ; Now check for Insert Legend Item.
    nLegends = 0
    oView = oWin->GetCurrentView()
    layerAnnotation = oView->GetByIdentifier("ANNOTATION LAYER")
    if (OBJ_VALID(layerAnnotation)) then begin
        ...
        for i=0, nAnnotations-1 do begin
            if (OBJ_ISA(annotations[i], "IDLITVISLEGEND")) then begin
                if nLegends eq 0 then $
                    oLegends = [annotations[i]] $
                else $
                    oLegends = [oLegends, annotations[i]]
                nLegends++
            endif
        endfor
    endif

    allowInsertLegendItem = 0
    for i=0,nSel-1 do begin
        ...
        if ((OBJ_ISA(oSel[i], 'IDLitVisPlot')) || $
            (OBJ_ISA(oSel[i], 'IDLitVisPlot3D')) || $
            (OBJ_ISA(oSel[i], 'IDLitVisContour')) || $
            (OBJ_ISA(oSel[i], 'IDLitVisSurface'))) then begin
            allowInsertLegendItem = 1  ; I'm in business
            break
        endif
    endfor
    id = self->GetFullIdentifier()+"/OPERATIONS/INSERT/LEGENDITEM"
    menu = self->GetByIdentifier(id)
    self->DoOnNotify, id, 'SENSITIVE', (nLegends && allowInsertLegendItem)
