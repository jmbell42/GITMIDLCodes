; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/itools/framework/idlitsrvstyles__define.pro#1 $
;
...
;
pro IDLitsrvStyles::RestoreAll

    compile_opt idl2, hidden

    oSys = self->GetTool()
    oStyleContainer = oSys->GetByIdentifier('/REGISTRY/STYLES/My Styles')
    if (OBJ_VALID(oStyleContainer)) then begin
        oOldStyles = oStyleContainer->Get(/ALL, COUNT=nstyles)
        oStyleContainer->Remove, /ALL
        ...
        oSys->CreateFolders, "Registry/Styles/My Styles"
    endelse
    oStyleContainer = oSys->GetByIdentifier('/REGISTRY/STYLES/System Styles')
    if (OBJ_VALID(oStyleContainer)) then begin
        oOldStyles = oStyleContainer->Get(/ALL, COUNT=nstyles)
        oStyleContainer->Remove, /ALL
        if (nstyles gt 0) then $
    ...
    ; User styles.
    if (~IDLitGetResource('styles', path, /USERDIR)) then $
        return
    styleFiles = FILE_SEARCH(path, '*_style.sav', COUNT=nstyles)

    for i=0,nstyles-1 do $
        self->Import, styleFiles[i], /NO_NOTIFY
...
;
pro IDLitsrvStyles::Duplicate, styleName
