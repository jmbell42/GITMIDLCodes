; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/itools/framework/_idlitmapprojection__define.pro#1 $
;
...
pro _IDLitMapProjection::_UpdateLimit
    ...
    'Alaska Conformal': newlimit = [50, -180, 80, -120]
    'Albers Equal Area': newlimit = [0, -180, 90, 180]
    'Equidistant Conic A': newlimit = [0, -180, 90, 180]
    'Equidistant Conic B': newlimit = [0, -180, 90, 180]
    'Gnomonic': if (clat eq 0) then $
        newlimit = [-60, -180 > (clon-60), 60, (clon+60)<180]
    'Lambert Conformal Conic': newlimit = [0, -180, 90, 180]
    'Mercator': newlimit = [-75, -180, 75, 180]
    'Miller Cylindrical': newlimit = [-75, -180, 75, 180]
    'Polar Stereographic': newlimit = self._hemisphere ? $
            [-90, -180, 0, 180] : [0, -180, 90, 180]
    'State Plane': newlimit = [clat-10, clon-10, clat+10, clon+10]
    'UTM': newlimit = [-90, self._limit[1], 90, self._limit[3]]
    else: begin
        ...
        'Polyconic': ; fall thru
        'Stereographic': if (clat eq 0) then $
...
@map_proj_init_commonblock
    ...
    if (N_ELEMENTS(semiMajor) eq 1) then $
        self._semiMajor = semiMajor

    if (N_ELEMENTS(semiMinor) eq 1) then $
    ...
    if (N_ELEMENTS(standardPar1) || N_ELEMENTS(standardPar2)) then begin
        ...
        'Albers Equal Area': begin
            if (sPar1 eq -sPar2) then begin
                ; Don't allow standard parallels to be equal and opposite.
                self->ErrorMessage, SEVERITY=1, $
                    self._projection + ': ' + $
                    IDLitLangCatQuery('Warning:MapProjection:StandardPars'), $
                    TITLE=IDLitLangCatQuery('Menu:Operations:MapProjection')
                sPar1 = self._standardPar1
                sPar2 = self._standardPar2
            endif
            break
            end
        endswitch
