; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/itools/framework/_idlitvisualization__define.pro#1 $
;
...
pro _IDLitVisualization::GetProperty, $
    ...
    if ARG_PRESENT(impactAlone) then $
        impactAlone = self.impactAlone
...
; @idlit_on_error2
    ...
    if (N_ELEMENTS(impactAlone) gt 0) then $
        self.impactAlone = KEYWORD_SET(impactAlone)
...
function _IDLitVisualization::GetXYZRange, $
    ...
    for i=0, nObjs-1 do begin
        ...
        if OBJ_ISA(oObj, '_IDLitVisualization') then begin
            ...
            oObj->_IDLitVisualization::GetProperty, $
                IMPACTS_RANGE=impactsRange, IMPACT_ALONE=impactAlone
    ...
    if (~success && impactIndex ge 0) then begin
        impactsRange = oObjList[impactIndex]->GetXYZRange( $
            xRange, yRange, zRange, DATA=bDataRange)
        if (impactsRange) then begin
            self->_AccumulateXYZRange, success, $
                outxRange, outyRange, outzRange, $
                xRange, yRange, zRange, $
...
pro _IDLitVisualization__Define
    ...
    struct = { _IDLitVisualization,       $
        ...
        impactAlone: 0b,                  $ ; Does vis. impact range if only item?
