...
pro thermo_readsat, filelist, $
                    data, time, nTimes, vars_t, nAlts_t, nSats, nFiles,version
    ...
    if (strlen(filelist(0)) gt 0) then begin
        ...
        if (strpos(filelist(0),'ALL') gt 0 or strpos(filelist(0),'GLO') gt 0 or $
            strpos(filelist(0),'USR')  gt 0 or strpos(filelist(0),'THM')  gt 0) then begin
            iSatPosStart = 7
         endif else begin
            if (strpos(filelist(0),'.dat') gt 0) then isatposstart = 8 else iSatPosStart = 10
...
;            nalts_t = 43
;            nvars_t = 38
;            data_t = data_t(0:nvars_t-1,*,*,0:nalts_t-1)
