...
if (reread) then begin
    ...
    gracedir = '~/UpperAtmosphere/GRACE/data/'+tostr(iyear)+'/'
...
;    realdate = date_conv(date,'r')
    ...
    GraceAltitude   = fltarr(nTimes)

    nGraceMax = 800000L
    GracePosition = fltarr(3,nGraceMax)
    GraceTime  = dblarr(nGraceMax)
    MassDensity = fltarr(nGraceMax)
    GraceWind = fltarr(nGraceMax)
    GraceLocalTime=fltarr(nGraceMax)
    t = ' '
    line = 0L
    ily = isleapyear(iyear)
    bday = doy
    if ily then ndy = 366 else ndy = 365
    for cday = 0, ndays - 1 do begin
        doy = bday + cday

        if doy gt ndy then begin
           doy = doy - ndy
        ...
        gracedir = '~/UpperAtmosphere/GRACE/data/'+tostr(iyear)+'/'
        grace_file_a = file_search(gracedir+'Density_graceA_3deg_'+yr+'_'+ $
          chopr('00'+tostr(doy),3)+'.ascii',count=nf)
        if nf gt 0 then begin
   ;     grace_file_w = gracedir+'Wind_3deg_'+yr+'_'+tostr(doy)+'.ascii'
        
        close,/all
        openr,1,grace_file_a
    ;    openr,2,grace_file_w
        readf,1,t
        readf,1,t
    ;    readf,2,t
        ...
        while (not eof(1)) do begin
            ...
            GraceTime(line) = seconds+ basetime
            GracePosition(0,line) = long
            GracePosition(1,line) = lat
            GracePosition(2,line) = height
            MassDensity(line) = density
            GraceLocalTime(line) = chlocaltime
;            GraceWind(line) = wind
    ...
    for iTime = 0L, nTimes-1 do begin
        ...
        dt = abs(time(iTime)-GraceTime)
        loc = where(dt eq min(dt))

        i = loc(0)
  
        GraceDensity(iTime)  = MassDensity(i)/1.e-12;*0.7
        GraceAltitude(iTime) = GracePosition(2,i)
        
        loc = where(GitmAlts gt GraceAltitude(iTime))
        i = loc(0)
        x = (GraceAltitude(iTime) - GitmAlts(i-1)) / $
...
;        print, GraceAltitude(iTime), GraceDensity(iTime), GitmDensity(iTime)/1.0e-12, $
;          (1.0 - x)*GitmAlts(i-1)+x*GitmAlts(i)

        loc = where(GitmAlts gt GraceAltitude(iTime)+h)
        i = loc(0)
        x = ((GraceAltitude(iTime)+h) - GitmAlts(i-1)) / $
          (GitmAlts(i) - GitmAlts(i-1))
    ...
    GraceLocalTime = GraceLocalTime(0:nTimes-1)
    ...
    for iTime = 0L, nTimes-1 do begin
        ...
        if (count gt 0) then begin
            ...
            GraceAve(iTime) = mean(GraceDensity(loc))
        endif
    ...
    nY = max(DayOrbitStart - DayOrbitEnd) + 1
        ...
        for i = 1, nPtsDay-1 do begin
            ...
            if (iFound) then begin
                ...
                if plotdiff then $
                  ...
                  GitmDensity(Day(i))
                cDay(iOrbit-1, iY) = GraceDensity(Day(i))
            endif
    ...
    nY = max(NightOrbitStart - NightOrbitEnd) + 1
    
    print, "night : ", nY, nNOrbits      ;here the "nOrbits" should be "nNOrbits" to refer to the night

    if (nNOrbits gt 0) then begin       ;"nOrbits" should be "nNorbits"
        ...
        for i = 1, nPtsNight-1 do begin
            ...
            if (iFound) then begin
                ...
                if plotdiff then $
                  ...
                  GitmDensity(Night(i))
                cNight(iNorbit-1, iY) = GraceDensity(Night(i))
            endif

        endfor

        for iOrbit = 0, nNOrbits-2 do begin       ;"nOrbits" should be "nNOrbits" to refer to night
...
yrange = mm([GraceDensity,GITMDensity])
;yrange = [0.0,40.0]
...
if not plotdiff then begin
    ...
    plot, time-stime, GraceDensity, yrange = yrange, pos = pos, $
    ...
    xyouts, t3, max(yrange) - 4.0, 'GRACE'
    
    
    plot, time-stime, GraceDensity-gitmdensity, $
    ...
    rmse = sqrt(mean((GraceDensity-gitmdensity)^2))
    rmsd = sqrt(mean((GraceDensity)^2))
    nrms = rmse/rmsd * 100.0
    
    pdif = mean((GraceDensity-gitmDensity)/GraceDensity) * 100.0
    ...
    xyouts, t3, max(yrange-max(yrange)/2) - 2.0, 'GRACE - GITM'+srms
...
;--------------------------------------------------------------------
    ...
    yrange = [0,10]
    plot, time-stime, GraceAve, /nodata,yrange = yrange, pos = pos, $
      xtickname = ['Mar, 2007',xtickname], xtitle = xtitle, xtickv = [0,xtickv], $
      xminor = xminor, xticks = xtickn+1, xstyle = 1, charsize = 1.2, $
      ytitle = 'Mass Density (10!E-12!N kg/m!E3!N)',   $
      thick = 3,ystyle=1

    oplot, time-stime, GitmAve, linestyle = 0, thick = 3
    oplot, time-stime, GraceAve, linestyle = 0, thick = 3,color=254
    
    legend, ['GITM','GRACE'],color=[0,254],linestyle=0,box=0,pos = [pos(2)-.25,pos(3)-.01],$
            /norm

    
;   plot, time-stime, GraceAve-gitmave, $
;     yrange = yrange-max(yrange)/2, pos = pos2, $
;     xtickname = xtickname, xtitle = xtitle, xtickv = xtickv, $
;     xminor = xminor, xticks = xtickn, xstyle = 1, charsize = 1.2, $
;     ytitle = 'Mass Density (10!E-12!N kg/m!E3!N)',   $
;     thick = 3, /noerase, ystyle = 1
;   
;   rmse = sqrt(mean((GraceAve-gitmave)^2))
;   rmsd = sqrt(mean((GraceAve)^2))
;   nrms = rmse/rmsd * 100.0
;   
;   pdif = mean((GraceAve-gitmave)/GraceAve) * 100.0
;   
;   srms = +' (nRMS: '+string(nrms,format = '(f5.1)')+'%, '
;   srms = srms+string(pdif,format = '(f5.1)')+'% Difference)'
;   
...
;--------------------------------------------------------------------
    ...
    plotct,254,ctpos,mm(levels),$
      'GRACE Dayside Mass Density (10!E-12!N kg/m!E3!N)',/right
...
;--------------------------------------------------------------------
    ...
    ctpos = [pos(2)+0.01,pos(1),pos(2)+0.03,pos(3)]
    plotct,254,ctpos,mm(levels),$
      'GRACE Nightside Mass Density (10!E-12!N kg/m!E3!N)',/right
...
;            it = where(gracedensity eq cday(iorbit,ind(0)))
...
;    endfor
;    
;    legend,[' Grace',' GITM'],psym = [4,1],color = [60,230],thick = 5,pos = [pos(0)+.03,pos(1) - .02]$
;      ,/norm, box = 0
;    
;    closedevice
;
