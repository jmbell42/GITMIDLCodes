...
filelist = file_search("t*.3D*.save")
...
for iFile = 0, nFiles-1 do begin
    ...
    if dotempunit then begin
        converttemp,data,nlons,nlats,nalts,temperature 
        data(4,*,*,*) = temperature
    endif

ivnn = where(vars eq ' Vn (north)')
ivin = where(vars eq ' Vi (north)')
ivne = where(vars eq ' Vn (east)')
ivie = where(vars eq ' Vi (east)')
ivn = where(vars eq ' Vn (magnitude)')
ivi = where(vars eq ' Vi (magnitude)')

vdiffn = data(ivnn,*,*,*) - data(ivin,*,*,*)
vdiffe = data(ivne,*,*,*) - data(ivie,*,*,*)
vdiffm = data(ivn,*,*,*) - data(ivi,*,*,*)

vdiff = fltarr(1,nlons,nlats,nalts)
for ilon = 0, nlons -1 do begin
    for ilat = 0, nlats - 1 do begin
        for ialt = 0, nalts - 1 do begin
            if vdiffm(0,ilon,ilat,ialt) lt 0 then fac = 1 else fac = -1
            vdiff(0,ilon,ilat,ialt) =  fac*sqrt(vdiffn(0,ilon,ilat,ialt)^2 $
                                                + vdiffe(0,ilon,ilat,ialt)^2)
        endfor
    endfor
endfor


vars(nvars-1) = 'Velocity Difference (Vn - Vi)'
data(nvars-1,*,*,*) = vdiff
data(ivnn,*,*,*) = vdiffn
data(ivne,*,*,*) = vdiffe
...
;cnt1 is a lon/alt plot
        ...
        plotvector = 'y' ;ask('whether you want vectors or not (y/n)','y')
...
; polar is variable to say whether we have polar plots or not
       if slice eq 1 then begin
           polar = 1
           ;polar = fix(ask("if polar (0,1): ",tostr(polar)))
       endif
...
; vn_cnt is whether to plot vectors of Vdiff
        vn_cnt = 1
