...
for iFile = 0, nFiles-1 do begin
    ...
    read_thermosphere_file, filename, nvars, nalts, nlats, nlons, $
      vars, data, rb, cb, bl_cnt,itimearr,version
...
; vn_cnt is whether to plot vectors of Vn
    ...
    data_avg=fltarr(nvars,nlons,nlats,nalts)
    for ivar = 0, nvars - 1 do begin
       for ialt = 0, nalts - 1 do begin
          for ilat = 0, nlats - 1 do begin
             data_avg(ivar,*,ilat,ialt) = mean(data(ivar,2:nlons-3,ilat,ialt))
          endfor
       endfor
    endfor

    thermo_plotzonal,cursor_x,cursor_y,strx,stry,step,nvars,sel,nfiles,	$
      ...
      cursor_cnt,data_avg,alt,lat,lon,	  $
