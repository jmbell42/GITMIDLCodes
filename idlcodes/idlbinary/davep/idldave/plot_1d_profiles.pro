if n_elements(d) eq 0 then d = ''
  ...
  files = file_search(directories(0)+'/*'+f1+'*')
  ...
  if nfiles_new gt 0 then begin
     ...
     for ifile = 0, nfiles_new - 1 do begin
        l1 = strpos(files(ifile),'/',/reverse_search)+1
        type = strmid(files(ifile),l1,5)
...
;         file = file_search(directories(idir)+'/'+'1DALL'+'*'+f+'*')
   ...
   if tempvar ge 0 then begin
      ...
      get_1d_profile,allfile,15,coordinates,profile
      tempvalue(*,idir) = profile

      get_1d_profile,allfile,4,coordinates,profile
      densvalue(*,idir) = alog10(profile)
      
      get_1d_profile,allfile,18,coordinates,profile
      vvelvalue(*,idir) = profile
      
      for mvar = 0, nmvars-1 do begin
         get_1d_profile,file,mvars(mvar),coordinates,profile
         mvalue(mvar,*,idir) = profile
      endfor

      for mvar = 0, ndvars-1 do begin
         get_1d_profile,file,dvars(mvar),coordinates,profile
         dvalue(mvar,*,idir) = profile
      endfor
...
plotd = 1
if plotd then begin
   cld = findgen(ndvars)*(245/(ndvars))+245/(ndvars)
   linestyle = intarr(ndvars)
 if ifile eq 0 then begin
      if n_elements(mind) eq 0 then begin
         mind = 0
         maxd = 0
      endif
      mind = float(ask('minimum dens to plot (0 for auto): ', tostrf(mind)))
      maxd = float(ask('maximum dens to plot (0 for auto): ', tostrf(maxd)))
   endif
   if mind eq 0 then minden = min(dvalue) else minden = mind
   if maxd eq 0 then maxden = max(dvalue) else maxden = maxd
   xrange = [minden,maxden]
...
for idir = 0, ndirs - 1 do begin
   for dvar = 0, ndvars - 1 do begin
      oplot,dvalue(dvar,*,idir),coordinates/1000.,color=cld(dvar),linestyle = linestyle(idir),$
      thick=3
   endfor
endfor

legend,vars(dvars),box = 0,colors=cld,pos=[pos(0)+.01,pos(3)-.01],/norm,$
  linestyle = 0,thick=3
endif
;-------------------------
plotvel=0
...
yrange = mm(alts)
  ...
  xtitle = 'Density [CO2]'
    yrange = mm(alts)
    if ifile eq 0 then begin
      if n_elements(minc) eq 0 then begin
         minc = 0
         maxc = 0
      endif
      minc = float(ask('minimum [co2] to plot (0 for auto): ', tostrf(minc)))
      maxc = float(ask('maximum [co2] to plot (0 for auto): ', tostrf(maxc)))
   endif
   if minc eq 0 then minco = min(densvalue) else minco = minc
   if maxc eq 0 then maxco = max(densvalue) else maxco = maxc
   xrange = [minco,maxco]
   get_position, ppp, space, sizes, 5, pos, /rect
    ...
    plot,fltarr(nalts),alts,/nodata,xrange = xrange,background=0,xtitle=xtitle,$
           ytitle=ytitle,yrange = yrange,pos=pos,/noerase,$
         ystyle=  1,xstyle = 1,ytickname=strarr(10)+' '
    oplot,densvalue,alts,color = 60,thick=3
...
if plotvvel then begin
  xtitle = 'Vertical Velocity'
   ...
   get_position, ppp, space, sizes, 3, pos, /rect
    ...
    plot,fltarr(nalts),alts,/nodata,xrange = xrange,background=0,xtitle=xtitle,$
           ytitle=ytitle,yrange = [20,50],pos=pos,/noerase,$
         ystyle=  1,xstyle = 1,ytickname=strarr(10)+' '
