...
;    realdate = date_conv(date,'r')
    ...
    for cday = 0, ndays - 1 do begin
        ...
        champdir = '~/UpperAtmosphere/CHAMP/data/'+tostr(iyear)+'/'
        champ_file_a = file_search(champdir+'Density_3deg_'+yr+'_'+ $
...
psfile_raw = 'ch_'+run+'_raw.ps'
psfile_ave = 'ch_'+run+'_ave.ps'
psfile_2dd = 'ch_'+run+'_2dd.ps'
psfile_2dn = 'ch_'+run+'_2dn.ps'
...
;--------------------------------------------------------------------
    ...
    yrange = [0,5]
    plot, time-stime, ChampAve, /nodata,yrange = yrange, pos = pos, $
      xtickname = xtickname, xtitle = xtitle, xtickv = xtickv, $
      xminor = xminor, xticks = xtickn, xstyle = 1, charsize = 1.2, $
      ytitle = 'Mass Density (10!E-12!N kg/m!E3!N)',   $
      thick = 3,ystyle=1
    ...
    oplot, time-stime, ChampAve, linestyle = 0, thick = 3,color=254
    
    legend, ['GITM','CHAMP'],color=[0,254],linestyle=0,box=0,pos = [pos(2)-.25,pos(3)-.01],$
            /norm
