...
files = ask('Directory containing files in the ensemble to plot',files)
filelist = findfile(files+'/3DALL*bin')

if (n_elements(basefiles) eq 0) then files = ''
basefiles = ask('Directory containing basefiles in the ensemble to plot',basefiles)
basefilelist = findfile(basefiles+'/3DALL*bin')
...
if (IsPolar) then begin
 ...
 pos(3) = pos(3)-.1
  pos(1) = pos(1)-.1
    ...
    xyouts,pos(2) - .14,pos(3)+.02,'February 15, 2011 02:50 UT',charsize=1.4,/norm
...
;  pos(3) = pos(3)-.1
;  pos(1) = pos(1)-.1
;  xyouts,pos(2) - .14,pos(3)+.02,'February 15, 2011 06:00 UT',charsize=1.4,/norm
;  mini2 = float(smini2)
;  maxi2 = float(smaxi2)
;  if (mini2 eq 0.0 and maxi2 eq 0.0) then begin
