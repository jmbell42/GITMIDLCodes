pro thermo_nmf2,cursor_x,cursor_y,strx,stry,step,nvars,sel,nfiles, $
...
;print, pos
        ...
        if (not polar) then begin
            if ortho ne 1 then begin
                !p.position = pos
;                map_set, title=variable+' % Difference at '+strmid(stime,0,15)+' UT'
                map_set, title=variable+' at '+strmid(stime,0,15)+' UT'
            endif else begin
                pos = [.1,.1,.57,.7]
                !p.position = pos
;                map_set, title=variable+' % Difference at '+$
;                  strmid(stime,0,15)+' UT',plat,plon,/ortho
                map_set, title=variable+'  at '+$
                  strmid(stime,0,15)+' UT',plat,plon,/ortho
            endelse
        endif else begin
...
;          xrange=xrange,yrange=yrange,$
;          /noerase, levels = linelevels, /follow, c_linestyle = linestyle,$
;          /over, c_thick = 2
...
;        xstyle=xstyle,ystyle=ystyle,$
;         xrange=xrange,yrange=yrange,$
;         /noerase, levels = linelevels, /follow, c_linestyle = linestyle,/over
