...
while not eof(1) do begin
    ...
    if ndays ne 1 then    champfile(ndays-1) = champ_end_file
    fcheck = file_search(champfile(0))
    if file_test(champfile(0)) then begin
    Density = [0.0]
    Lon = [0.0]
    Lat = [0.0]
    Alt = [0.0]
    ...
    for i = 1, nPtsDay-1 do begin
        ...
        if (day(i)-day(i-1) gt 1) then begin
            if (nOrbits eq 0) then begin
                ...
                if (day(i)-day(i-1) gt 14) then begin
                    DayOrbitStart = [DayOrbitStart,day(i)]
                    DayOrbitEnd   = [DayOrbitEnd  ,day(i-1)]
                endif
            endelse
            if (day(i)-day(i-1) gt 14) then nOrbits = nOrbits+1
        endif
    ...
    for i = 1, nPtsNight-1 do begin
        ...
        if (night(i)-night(i-1) gt 1) then begin
            if (nNorbits eq 0) then begin
                ...
                if (night(i)-night(i-1) gt 14) then begin
                    NightOrbitStart = [NightOrbitStart,night(i)]
                    NightOrbitEnd   = [NightOrbitEnd  ,night(i-1)]
                endif
            endelse
            if (night(i)-night(i-1) gt 14) then nNorbits = nNorbits+1
        endif
...
minday = 90
minnight = 91
loc = where(sza le minday or sza ge minnight,count)
;loc = where(sza eq sza)
locday = where(sza le minday,countday)
locnight = where(sza ge minnight,countnight)
...
;    levels = findgen(61) * 15.0/60.0
    ...
    levels = findgen(61) * (max(cnight)+max(cday)/2)/60.0 
