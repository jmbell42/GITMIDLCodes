pro zensun,date,time,lat,lon,zenith,azimuth,solfac,sunrise=sunrise, $
           sunset=sunset,local=local,latsun=latsun,lonsun=lonsun
print, 'USE zsun, this doesnt work for some reason!!!'
...
td = strsplit(date,'-',/extract)

idate(0) = td(0)
idate(1) = td(1)
idate(2) = td(2)
idate(3) = floor(time)
idate(4) = floor((time - floor(time))*60)
idate(5) = 0

c_a_to_r, idate, currenttime
date = date(0)
...
Tilt = 23.5
DaysPerYear = 365.25
SecondsPerYear = DaysPerYear * 24.0*3600.0
...
itime(0) = iVernalYear
itime(1) = iVernalMonth
itime(2) = iVernalDay
itime(3) = iVernalHour
itime(4) = iVernalMinute
itime(5) = iVernalSecond
...
LocalTime = (time/3600.0 + (!dtor*Lon) * HoursPerDay / (2*!pi)) 
...
zenith =  acos(SinDec*sin(lat*!dtor) + $
             CosDec*Cos(lat*!dtor) * $
             cos(!pi*(LocalTime-HoursPerDay/2)/(HoursPerDay/2)))/!dtor
