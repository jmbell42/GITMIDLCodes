...
if (GetNewData) then begin
    ...
    thermo_readsat, filelist_new, data, time, nTimes, Vars, nAlts, nSats, Files,version
...
;if (nSats eq 1) then begin
  ...
  if strpos(filelist_new(0),'ALL') gt 0 then begin
...
new_title='Temp'
    ...
    if (strpos(nomap,'n') lt 0) then begin
        ...
        if lons(0) ne lons(npts-1) and lats(0) ne lats(npts-1) then begin
           xyouts, lons(0), lats(0), 'Start', charsize = 1.2
           xyouts, lons(nPts-1), lats(nPts-1), 'End', charsize = 1.2
        endif else begin
           xyouts, lons(0), lats(0), 'Location', charsize = 1.2        
        endelse
     endif
...
if pres then begin
    ...
    volume = (6378000+(alts(0,2:nalts-3)*1000.0))^2*sin(90*!dtor)*(5*!dtor)^2* $
      ((alts(0,2:nalts-3)*1000.0)-(alts(0,1:nalts-4)*1000.0))
    it = nfiles-1
     nco2 =alog10(reform(data(0,it,4,2:nalts-3)))
     no2 = alog10(reform(data(0,it,8,2:nalts-3)))
     no = alog10(reform(data(0,it,6,2:nalts-3)))
     nn2 = alog10(reform(data(0,it,7,2:nalts-3)))
     nar = alog10(reform(data(0,it,9,2:nalts-3)))
     nco = alog10(reform(data(0,it,5,2:nalts-3)))
;     nco2 =alog10(reform(data(0,it,4,2:nalts-3)))
;     no2 = alog10(reform(data(0,it,11,2:nalts-3)))
;     no = alog10(reform(data(0,it,6,2:nalts-3)))
;     nn2 = alog10(reform(data(0,it,7,2:nalts-3)))
;     nar = alog10(reform(data(0,it,10,2:nalts-3)))
;     nco = alog10(reform(data(0,it,5,2:nalts-3)))
    n = reform(data(0,it,4,2:nalts-3))+reform(data(0,it,5,2:nalts-3))+ $
      reform(data(0,it,6,2:nalts-3))+ $
      reform(data(0,it,7,2:nalts-3))+reform(data(0,it,8,2:nalts-3))
    k = 1.38065e-23
    t = reform(data(0,it,15,2:nalts-3))
    p = n*k*t
...
loadct,39    
    ...
    plot,p, alts(0,2:nalts-3),yrange = [00,300],pos=pos,xtitle = 'Pressure (Pa)',$
      ytitle = 'Altitude',/xlog,thick=3

  get_position, ppp, space, sizes, 1, pos, /rect
 plot,t, alts(0,2:nalts-3),yrange = [00,300],pos=pos,xtitle = 'Temperature (K)',$
      /noerase,ytickname = strarr(10)+' ',thick=3
cl = get_colors(6)
 get_position, ppp, space, sizes, 2, pos, /rect
 plot,nco2, alts(0,2:nalts-3),yrange = [00,300],pos=pos,xtitle = 'Log(Density m!U-3!N)',$
      /noerase,ytickname = strarr(10)+' ',/nodata,xrange = [7,24],xstyle = 1
 oplot, nco2,alts(0,2:nalts-3),color = cl(0),thick=3
 oplot, nco,alts(0,2:nalts-3),color = cl(1),thick=3
 oplot, no2,alts(0,2:nalts-3),color = cl(2),thick=3
 oplot, no,alts(0,2:nalts-3),color = cl(3),thick=3
 oplot, nn2,alts(0,2:nalts-3),color = cl(4),thick=3
 oplot, nar,alts(0,2:nalts-3),color = cl(5),thick=3
...
;    dt = deriv(alts(0,ialt1-1:ialt1+1)*1000.0,value(i,ialt1-1:ialt1+1))
;    dv(i) = dt(1)
;endfor
