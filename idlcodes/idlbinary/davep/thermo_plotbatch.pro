...
;                                 (lon(imin) - (xt))/(lon(imin)-lon(imin+1)))
           ...
           minlon = min(ltlons,imini)

            xtickv = shift(ltlons,-imini)
            xtickname = tostr(shift(ticknames,-imini))
            xtickname = [xtickname,xtickname(0)]
