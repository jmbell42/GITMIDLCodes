...
pro thermo_plotzonal,cursor_x,cursor_y,strx,stry,step,nvars,sel,nfiles, $
...
if cnt2 eq 1 then begin
    ...
    maxi=max(datatoplot)
    mini=min(datatoplot)
    x=reform(lon(*,selset,2:nalts-3))
    y=reform(alt(*,selset,2:nalts-3))
...
if cnt3 eq 1 then begin
    datatoplot=reform(data(sel,selset,*,2:nalts-3))
    maxi=max(datatoplot)
    mini=min(datatoplot)
    x=reform(lat(selset,*,2:nalts-3))
    y=reform(alt(selset,*,2:nalts-3))
    ...
    if ghostcells eq 1 then begin
        xrange=mm(lat)
        yrange=mm(alt(2:nalts-3))
    endif
    if ghostcells eq 0 then begin
        backup_xrange=mm(lat)
        backup_yrange=mm(alt(0,0,2:nalts-3))
        default_xrange=[-90,90]
        default_yrange=mm(alt(0,0,2:nalts-3))
;If out of range then use 'mm' to set xrange and yrange values.
;Else use default values.
        if (backup_xrange[0] lt default_xrange[0]) $
          and (backup_xrange[1] gt default_xrange[1]) then begin
            xrange=mm(lat)
            yrange=mm(alt(0,0,2:nalts-3))
        endif else begin
            xrange=[-90,90]
            yrange=mm(alt(0,0,2:nalts-3))
        endelse
        ...
        endelse
         ...
         if (not polar) then begin
            if ortho ne 1 then begin
                !p.position = pos
                map_set, title='Zonal average '+variable+' at '+$
                ...
                !p.position = pos
                map_set, title='Zonal average '+variable+' at '+$
                  strmid(stime,0,15)+' UT ',plat,plon,/ortho,/noborder
            endelse
        endif else begin
...
;            makect,'mid'
        ...
        contour,d2, x2, y2,POS=pos,$
          ...
          xrange=xrange,yrange=yrange,$
          c_colors=clevels,$
          xtitle='Latitude',ytitle=ytitle,/cell_fill,/noerase
 if cnt2 then begin
...
;          xstyle =5, ystyle=5, color = 0, thick=1.5
        plot, [-180,180],[-90,90],xstyle=1,ystyle=1,/noerase,/nodata,pos=pos, $
          xtitle = 'Longitude', ytitle = 'Latitude',charsize = charsize
    endif
