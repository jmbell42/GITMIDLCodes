;Normal inputs
;full path for imf file
;normally i
; 'imfyyyymmdd.dat'
;full path for solar wind file
;d

acedir = '/bigdisk1/ridley/Data/cdaweb.gsfc.nasa.gov/pub/istp/ace/'

if n_elements(mfilein) eq 0 then mfilein = ''
mfilein  = ask('file to convert',mfilein)
filein=mfilein
if n_elements(satfile) eq 0 then satfile = 's'
satfile = ask('whether this is a satellite file or an IMF file (s/i)',satfile)  
if n_elements(fileout) eq 0 then fileout = ''
fileout = ask('file to output',fileout)
...
;  magfile = acedir+'/mfi_h0/'+filein
;  swfile  = acedir+'/swe_h0/'+ask('SOLAR WIND file to convert','')

  magfile = filein
  if n_elements(swfile1) eq 0 then swfile1 = ''
  swfile1  = ask('SOLAR WIND file to convert',swfile1)

  swfile = findfile(swfile1)
  swfile = swfile(0)
      ...
      if (abs(magfield(0,i)) lt 1000.0) then begin
          ...
          while (not done) do begin
              ...
              if (swtime(j) gt magtime(i) and $
                  sw(0,j) gt -5000.0 and sw(1,j) gt -5000. and $
  ...
  if (strpos(dynamic,'d') gt -1) then begin
      ...
      print, "mean x          : ", mean(xyz(0,*))
      print, "mean vx         : ", mean(vel(0,*))
      print, "Mean time delay : ",-mean((xyz(0,*)-32.0*6372.0)/vel(0,*))
