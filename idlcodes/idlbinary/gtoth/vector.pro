...
PRO VECTOR,U,V,XX,YY,NVECS=nvecs,MAXVAL=maxval,LENGTH=length,HEAD=head,$
		  ...
		  xrange=xrange, yrange=yrange, pos=pos, xtickname=xtickname
...
if n_elements(yrange)   eq 0 then yrange   = [min(yy),max(yy)]
if n_elements(xtickname) eq 0 then xtickname = strarr(30)
if n_elements(pos)      eq 0 then usepos = 0 else usepos = 1
...
if n_elements(xx) gt 2 and n_elements(yy) gt 2 then begin
   ...
   maxi = max(abs(xx))
   loc = where(abs(xx) ne maxi)
   if (maxi/max(abs(xx(loc))) gt 10.0) then begin

     ; this means that something is messed up

     if (abs(min(xx)) eq maxi) then begin
        loc2 = where(xx eq min(xx))
        xx(loc2) = min(xx(loc))
     endif else begin
        loc2 = where(xx eq max(xx))
        xx(loc2) = max(xx(loc))
     endelse
   ...
   maxi = max(abs(yy))
   loc = where(abs(yy) ne maxi)
   if (maxi/max(abs(yy(loc))) gt 10.0) then begin

     ; this means that something is messed up

     if (abs(min(yy)) eq maxi) then begin
        loc2 = where(yy eq min(yy))
        yy(loc2) = min(yy(loc))
     endif else begin
        loc2 = where(yy eq max(xx))
        yy(loc2) = max(yy(loc))
     endelse
...
if (usepos) then begin
  plot,[xmin,xmax],[ymin,ymax],/nodata,XSTYLE=1,YSTYLE=1,NOERASE=noerase, $
       xrange = xrange, yrange = yrange, xtickname = xtickname, pos = pos
endif else begin
  plot,[xmin,xmax],[ymin,ymax],/nodata,XSTYLE=1,YSTYLE=1,NOERASE=noerase, $
       xrange = xrange, yrange = yrange, xtickname = xtickname
endelse
