...
pro read_thermosphere_file_3gc, filelist, nvars, nalts, nlats, nlons, $
  ...
  if (nBLKlat*nBLKlon gt nfiles) then begin
    ...
    for n=0,nBLKlat*nBLKlon-1 do begin
      ...
      if (n eq 0) then begin
          nlo = (nlons-6) * nBLKlon + 6
          nla = (nlats-6) * nBLKlat + 6
          data = dblarr(nvars, nlo, nla, nalts)
          tmp = dblarr(nvars)
      ...
      for k = 0, nalts-1 do begin
          for j = 0, nlats-1 do begin
              for i = 0, nlons-1 do begin
                  ...
                  ii = (n mod nBLKlon)*(nlons-6) + i
		  jj = (n / nBLKlon)*(nlats-6) + j
                  if (i ge 3 and i le nlons-4 and $
                      j ge 3 and j le nlats-4) then $
                    data(*,ii,jj,k) = tmp
                  if (jj lt 3) then data(*,ii,jj,k) = tmp
                  if (ii lt 3) then data(*,ii,jj,k) = tmp
                  if (jj gt nla-4) then data(*,ii,jj,k) = tmp
                  if (ii gt nlo-4) then data(*,ii,jj,k) = tmp
              endfor
          endfor	
      endfor
      close,1
