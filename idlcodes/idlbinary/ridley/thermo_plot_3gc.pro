pro thermo_plot_3gc,cursor_x,cursor_y,strx,stry,step,nvars,sel,nfiles, $
...
;    endif
    ...
    ; with 3 gc:  0:nLons
    ;  gc = [0, 1, 2] and gc = [nLons-3,nLons-2,nLons-1]
    ;  data without gc:  0:nLons-6  -> note we have an extra point for wrap around
    ; 
    if (polar) then begin
        ;print, 'polar'
        ;for i = 3, nLons -3 do begin
        ;    print, 'POLAR: i, lon =', i, lon(i,0,0)
        ;    ;print, 'SUBSETSIZE:  =', n_elements( lon(3:nLons-3,0,0))
        ;endfor; i = 3, nLons -3 do begin
        ;sublons = reform(lon(3:nLons-3,0,0))
        ;for i = 0, nLons - 6 do begin
        ;    print, 'SUBLONS(i) = ', i, sublons(i)
        ;endfor ;i = 0, nLons - 6 do begin
        ;print, 'SUBSET_SIZE:  =', n_elements( lon(3:nLons-3,0,0))
        ;print, 'nLons-6:     =', nLons-6
        datatoplot=reform(data(sel,3:nLons-3,loc,selset))
        ; wrap data around
        datatoplot(nLons-6,*) = datatoplot(0,*)
    ...
    if (polar) then begin
        ...
        if (npolar) then begin
            x = reform( (90.0 - lat(3:nLons-3,loc,selset)) * $
                        cos((lon(3:nLons-3,loc,selset)+utrot)*!pi/180.0 - !pi/2.0))
            y = reform( (90.0 - lat(3:nLons-3,loc,selset)) * $
                        sin((lon(3:nLons-3,loc,selset)+utrot)*!pi/180.0 - !pi/2.0))
        endif else begin
            x = reform( (90.0 + lat(3:nLons-3,loc,selset)) * $
                        cos((lon(3:nLons-3,loc,selset)+utrot)*!pi/180.0 - !pi/2.0))
            y = reform( (90.0 + lat(3:nLons-3,loc,selset)) * $
...
;levels = findgen(100)/30.0*(maxi-mini) + mini
